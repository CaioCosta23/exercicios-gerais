...
tPartida LePartida() {
    ...
    if (partida == NULL) {
        printf("Erro na alocacao de memoria da partida ");
        return partida;
    }

    partida->time1 = (char*)malloc(MAX_NOME * sizeof(char));
    if ((*partida).time1 == NULL) {
        printf("Erro na alocacao de memoria do time 1 da partida ");
        LiberaPartida(partida);
        return partida;
    }
    partida->time2 = (char*)malloc(MAX_NOME * sizeof(char));
    if ((*partida).time2 == NULL) {
        printf("Erro na alocacao de memoria do time 2 da partida ");
        LiberaPartida(partida);
        return partida;
    }
...
void ImprimePartida(tPartida p) {
    // Função inutilizada (devido ao formato do exercício e do programa);
}
